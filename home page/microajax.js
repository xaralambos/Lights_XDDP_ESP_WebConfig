...
function lightInF() {
  var myRequest;
  if(document.getElementById("lightIn").checked) {
    myRequest = new Request('/api/?gpio12=ON'); 
  }else{
    myRequest = new Request('/api/?gpio12=OFF'); 
  }
...
function lightOutF() {
  var myRequest;
  if(document.getElementById("lightOut").checked) {
    myRequest = new Request('/api/?gpio13=ON'); 
  }else{
    myRequest = new Request('/api/?gpio13=OFF'); 
  }
  
  fetch(myRequest); 
}

function requestContent02()
{
  microAjax("contenr.html", function (res)
  {
    document.getElementById("ESP02").innerHTML  = res;
  });
